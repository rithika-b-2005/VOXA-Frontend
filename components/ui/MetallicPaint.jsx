...
export async function parseLogoFromUrl(url) {
...
export default function MetallicPaint({ imageData, params = defaultParams }) {
  ...
  }, [isReady, params.speed]);

  useEffect(() => {
    const canvasEl = canvasRef.current;
    const gl = glRef.current;
    const uniforms = uniformsRef.current;
    if (!canvasEl || !gl || !uniforms || !imageData || !isReady) return;
  ...
  useEffect(() => {
    const gl = glRef.current;
    const uniforms = uniformsRef.current;
    if (!gl || !uniforms || !imageData || !isReady) return;
